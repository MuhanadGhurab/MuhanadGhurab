...
def test_file_sizes() -> None:
    limits = {
        "cyber-crow-command-center.svg": 500_000,
        "cyber-crow-command-center-static.svg": 300_000,
        "enterprise-orbit-diagram.svg": 250_000,
        "portfolio-telemetry.svg": 250_000,
    }
    for name, limit in limits.items():
        path = PROFILE / name
        assert path.exists(), name
        assert path.stat().st_size <= limit, f"{name} too large"
...
def test_renderer_deterministic() -> None:
    ...
    assert mid == after
    # first run may rewrite same content
...
def test_required_readme_strings() -> None:
    ...
    assert "cyber-crow-command-center.svg" in readme
    assert "Conceptual Portfolio Architecture" in readme or "conceptual map" in readme.lower()
    ...
    # no linked missing PDF
    assert "Muhanad-Ghurab-ATS-Resume.pdf)" not in readme.replace("\\", "/")
